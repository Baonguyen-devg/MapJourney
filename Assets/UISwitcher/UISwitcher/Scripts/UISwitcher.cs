...
namespace UISwitcher {
	public class UISwitcher : UINullableToggle {
        ...
        {
            tipRect.DOAnchorMin(anchor, 0.5f).SetEase(Ease.OutQuad);
            tipRect.DOAnchorMax(anchor, 0.5f).SetEase(Ease.OutQuad);
            tipRect.DOPivot(anchor, 0.5f).SetEase(Ease.OutQuad);
        }
    }
}